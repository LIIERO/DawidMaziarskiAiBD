...
def film_in_category(category_id:int)->pd.DataFrame:
    ...
    sel = f"""SELECT film.title, language.name as languge, category.name as category FROM film
          INNER JOIN film_category ON (film.film_id = film_category.film_id)
          INNER JOIN category ON (film_category.category_id = category.category_id)
          INNER JOIN language ON (film.language_id = language.language_id)
          WHERE film_category.category_id = {category_id} ORDER BY film.title"""
    df = pd.read_sql_query(sel, con=connection)
    return df
...
def number_films_in_category(category_id:int)->pd.DataFrame:
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    if not isinstance(category_id, int): return None # film -> film_category -> category zamiast film_list
    sel = f"""SELECT fl.category, COUNT(fl.title) FROM film_list fl
              INNER JOIN category ON (category.name = fl.category)
              WHERE category.category_id = {category_id} GROUP BY fl.category"""

    #sel = f"""SELECT category.name, COUNT(film.title) FROM film
...
def number_film_by_length(min_length: Union[int,float] = 0, max_length: Union[int,float] = 1e6 ) :
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    if not isinstance(min_length, (int, float)) or not isinstance(max_length, (int, float)): return None
    if max_length < min_length: return None
    sel = f"""SELECT length, COUNT(title) FROM film
              WHERE length<={max_length} AND length>={min_length} GROUP BY length"""
    df = pd.read_sql_query(sel, con=connection)
    return df
...
def client_from_city(city:str)->pd.DataFrame:
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    if not isinstance(city, str): return None
    sel = f"""SELECT city.city, customer.first_name, customer.last_name FROM city
...
def avg_amount_by_length(length:Union[int,float])->pd.DataFrame:
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    if not isinstance(length, (int, float)): return None
    sel = f"""SELECT film.length, AVG(payment.amount) FROM film
...
def client_by_sum_length(sum_min:Union[int,float])->pd.DataFrame:
    ...
    pd.DataFrame: DataFrame zawierający wyniki zapytania
    '''
    if not isinstance(sum_min, (int, float)): return None
    sel = f"""SELECT customer.first_name, customer.last_name, sum(film.length) FROM film
              INNER JOIN inventory ON (inventory.film_id = film.film_id)
              INNER JOIN rental ON (rental.inventory_id = inventory.inventory_id)
...
def category_statistic_length(name:str)->pd.DataFrame:
    ...
    sel = f"""SELECT category.name as category, AVG(film.length), SUM(film.length), MIN(film.length), MAX(film.length) FROM film
              INNER JOIN film_category ON (film_category.film_id = film.film_id)
              INNER JOIN category ON (category.category_id = film_category.category_id)
